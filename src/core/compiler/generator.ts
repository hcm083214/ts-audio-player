...
import { ASTElement, ASTRoot, ASTNode, ASTInterpolation, ASTText } from './parser'
...
export function generate(ast: ASTRoot): string {
  ...
  function genNode(node: ASTNode): string {
    if (node.type === 'Root') {
      const rootNode = node as ASTRoot;
      // 如果只有一个子节点，直接返回该节点的代码
      if (rootNode.children.length === 1) {
        return genNode(rootNode.children[0]);
      }
      // 如果有多个子节点，用数组包裹
      return `[${rootNode.children.map(genNode).join(',')}]`;
    } 
    else if (node.type === 'Element') {
      return genElementContent(node as ASTElement);
    } 
    else if (node.type === 'Interpolation') {
      const interpNode = node as ASTInterpolation;
      // 插值表达式需要访问 .value 来解包 Ref
      return `String(ctx.${interpNode.content}?.value ?? ctx.${interpNode.content})`;
    } 
    else if (node.type === 'Text') {
      const textNode = node as ASTText;
      // 转义特殊字符，防止生成的代码出现语法错误
      const escapedContent = textNode.content
        .replace(/\\/g, '\\\\')
        .replace(/'/g, "\\'")
        .replace(/"/g, '\\"')
        .replace(/\n/g, '\\n')
        ...
        const generateScopedChildren = (children: ASTNode[]): string => {
          return children.map((child: ASTNode) => {
            if (child.type === 'Interpolation') {
              const interpChild = child as ASTInterpolation;
              // 检查插值表达式是否引用了循环变量
              const content = interpChild.content;
              ...
              const elemChild = child as ASTElement;
              // 递归处理嵌套元素，生成 h() 调用
              // 注意：这里简化处理，不处理嵌套元素的 v-for/v-if
              const nestedPropsEntries: string[] = [];
              for (const key in elemChild.props || {}) {
                const val = elemChild.props[key];
                if (key.startsWith(':')) {
              ...
              const nestedChildrenStr = elemChild.children.length ? `[${generateScopedChildren(elemChild.children)}]` : '[]';
              return `h('${elemChild.tag}', ${nestedPropsStr}, ${nestedChildrenStr})`;
            } else if (child.type === 'Text') {
              const textChild = child as ASTText;
              const escapedContent = textChild.content
                .replace(/\\/g, '\\\\')
                .replace(/'/g, "\\'")
                .replace(/"/g, '\\"')
                .replace(/\n/g, '\\n')
